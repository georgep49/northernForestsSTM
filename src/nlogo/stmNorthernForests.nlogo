__includes["mrc.nls" "regen-mtx.nls" "pathogens.nls"]
extensions [profiler matrix table]
...
to setup
 ...
 set base-changes (list 20 20 30 80 120 -999);; -> this is time spent in each state
  set base-change-dict table:from-list [
    ["gr" 20]
    ["d-gr" 20]
    ["m-sh" 30]
    ["ksh-k" 80]
...
to init-patch-dynamics
  ...
  [
    ...
    set fire-history []
    set next-change table:get base-change-dict class
...
to thin-regenbank
  ...
  [
    ...
    [
      if regenbank-3 > 0
      [
        set regenbank-3( map [i -> rbinom i (1 - sap-mortality)] regenbank-3)
      ]

      if regenbank-4 > 0
      [
        set regenbank-4( map [i -> rbinom i (1 - sap-mortality)] regenbank-4)
      ]
