...
to setup
  set elevation-dataset gis:load-dataset "volcano.asc"
  set patch-grain 10
end
...
to calculate-slope-aspect-horn
  ...
  let dz sqrt ((dz_ew / patch-grain) ^ 2 + (dz_ns / patch-grain) ^ 2)

  set slope-horn atan dz patch-grain * (pi / 180)
...
509
NIL
ask patches [\nset pcolor scale-color grey hs 0 255] 
NIL
1
T
OBSERVER
