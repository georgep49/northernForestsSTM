...
to build-gf
  ...
  sr:set "autocorr_range" autocorr_range
  sr:set "mag_var" mag_var
  sr:set "nug" nug

  (sr:run
    "gaussian_field <- NLMR::nlm_gaussianfield(ncol, nrow, autocorr_range, mag_var, nug)"
    "gf <- terra::rast(gaussian_field)"
  )

  let x sr:runresult "rep(seq(0, ncol - 1), nrow)"
  let y sr:runresult "rep(seq(nrow - 1, 0), each = ncol)"
  let gf sr:runresult "raster::values(gf)"
...
sr:setup
  ...
  sr:set "ncol" max-pxcor + 1
  sr:set "nrow" max-pycor + 1
  sr:set "fractal_dim" fractal_dim
...
to fill-patches [xc yc zc]
  ...
  (foreach xc yc zc [[x y z] -> ask patch x y  [ set grad z ]])

  ask patches [
    set pcolor palette:scale-gradient [[236 226 240] [28 144 153]] grad 0 1
  ]
end
@#$#@#$#@
GRAPHICS-WINDOW
