...
to test
  ...
  let tmx (list [0.9 0.0] [0.05 0.99])    ;; transitions from 1.5m -> 5 cm dbh individuals
  let N (list 10 5)

  let AA  (map[ [a b] -> random-binomial a b ] N item 0 tmx)
  let BB  (map[ [a b] -> random-binomial a b ] N item 1 tmx)
...
to test2
    ...
    show item 1 (item 0 windModifiers)
end
