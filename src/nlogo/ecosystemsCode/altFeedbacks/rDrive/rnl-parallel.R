...
sfInit(parallel=TRUE, cpus=processors, type="SOCK")
sfClusterSetupRNG()
...
names(params) <- c("from-r", "max-ticks", "burn-in-regen", "fire-frequency", "frac-consumed", "seed-pred",
                   ...
                   "crit-density-yng", "crit-density-old", "invasion")
...
r <- 1:n.runs
sfClusterEval(ls())
sfSource('rnlFunctions.R')
#print(date())
result.par <- sfClusterApplyLB(r, sim.default) 
#print(date())
